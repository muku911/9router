...
import { OAUTH_PROVIDERS, APIKEY_PROVIDERS } from "../constants/providers";
import { MEDIA_PROVIDER_KINDS, AI_PROVIDERS } from "../constants/providers";
import { translate } from "../i18n";

const getPageInfo = (pathname) => {
  if (!pathname) return { title: "", description: "", breadcrumbs: [] };

  // Media provider detail: /media-providers/[kind]/[id]
  ...
  if (pathname.includes("/mitm"))
    return { title: "MITM Proxy", description: "Intercept CLI tool traffic and route through 9Router", icon: "security", breadcrumbs: [] };
  ...
  if (pathname.includes("/skills"))
    return { title: "Agent Skills", description: "Copy a link and paste to your AI to use 9Router — no install needed", icon: "extension", breadcrumbs: [] };
...
export default function Header({ onMenuClick, showMenuButton = true }) {
  ...
  return (
    <header className="shrink-0 flex items-center justify-between gap-3 px-4 lg:px-8 pt-3 pb-2 border-b border-border-subtle bg-surface/40 backdrop-blur-md z-20">
      {/* Mobile menu button */}
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {/* Donate button */}
        <a
          href="https://9router.com/api/donate"
          target="_blank"
          rel="noreferrer"
          className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-pink-500/10 hover:bg-pink-500/20 text-pink-600 dark:text-pink-400 text-xs font-semibold transition-colors"
        >
  ...
  return (
    <div className="relative w-[160px] sm:w-[220px]">
      ...
      <input
        ...
        className="w-full h-8 pl-7 pr-7 rounded-md border border-border bg-surface-2/45 text-sm focus:outline-none focus:border-brand-500/30 focus:shadow-[0_0_10px_rgba(229,106,74,0.08)] transition-all duration-300 placeholder:text-text-muted/60"
      />
      {query && (
        <button
          type="button"
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-50 w-48 rounded-lg border border-border bg-surface shadow-[var(--shadow-elev)] py-1 slide-in-top">
            <button
              onClick={() => { setOpen(false); navigate("/profile"); }}
              className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-main hover:bg-surface-2 transition-colors"
            >
